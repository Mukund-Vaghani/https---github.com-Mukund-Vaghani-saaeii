...
router.get('/getcart', function (req, res) {
    var id = req.user_id;
    middleware.decryption(req.body, function (request) {
        product.cartDetails(request, id, function (code, message, data) {
            middleware.send_response(res, req, code, message, data);
        });
    })
});

router.post('/product_listing', function (req, res) {
    var id = req.user_id;
    middleware.decryption(req.body, function (request) {
        var rules = {
            market_id: 'required',
        ...
        if (middleware.checkValidationRules(res, request, rules, messages)) {
                 product.product_listing (request,id, function (code, message, data) {
                middleware.send_response(res, req, code, message, data);
            });
        };
    })
